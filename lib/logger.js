...
function isBotSession(sessionRef) {
  return typeof sessionRef === 'string' && sessionRef.startsWith('bot-');
}
...
const Log = {
  event(key, sessionRef, message, extra) {
    ...
    if (isBotSession(sessionRef)) return;
  ...
  http(method, url, status, ms, _sessionRef, botLabel) {
    if (status < 400) return;
    const statusCol = status >= 500 ? C.red : C.yellow;
    const statusStr = c(statusCol, String(status));
    const tail = botLabel ? c(C.dim, ` [${botLabel}]`) : '';
    const line = `[${c(C.dim, ts())}] ${statusStr} ${method} ${url} ${c(C.dim, `${ms}ms`)}${tail}`;
