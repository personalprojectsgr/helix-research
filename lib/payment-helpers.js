...
async function firePurchaseCAPI(order, req) {
  ...
  const userData = tracking.buildUserData({
    ...
    externalId: order.customer.email || order.id,
  });
  const customData = {
  ...
  Log.event('purchase_capi', order.tracking?.sessionRef,
    `${order.id} $${order.totalUsd.toFixed(2)} ${result.ok ? 'CAPI ok' : 'CAPI skipped'}`);
}
